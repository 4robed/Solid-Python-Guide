...
A Class should have one job. If a class has more than one responsibility, it becomes coupled. A change to one
...
management and animal properties management. The constructor and get_name mange the Animal properties while the save 
...
Animal properties will have to be touched and recompiled to compensate for the new changes.
...
"""
When designing our classes, we should aim to put related features together, so whenever they tend to change they 
change for the same reason. And we should try to separate features if they will change for different reasons. 
- Steve Fenton - 
"""

"""
The downside of the solution is that the clients of this code have to deal with two classes. A common solution to 
this dilemma is to apply the Facade pattern. Animal class will be the Facade for animal database management and 
